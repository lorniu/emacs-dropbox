...
(defvar dropbox-http-backend pdd-backend)
...
(defun dropbox-request (url &rest args)
  (declare (indent 1))
  (pdd-then (unless (string-match-p "/oauth2" url) (dropbox-get-token))
    (lambda (token)
      (let ((pdd-headers (if token `((bear ,token))))
            (pdd-fail (lambda (r) (message "[ERR] %s" r)))
...
(defun dropbox--download (path &optional zip?)
  ...
  (dropbox-request (concat "https://content.dropboxapi.com/2/files/download" (if zip? "_zip"))
    :headers `(t (Dropbox-API-Arg . ,(encode-coding-string (json-encode `(("path" . ,path))) 'utf-8)))))
            ...
            (unwind-protect
                (progn
                  ...
                  (let* ((access-code (read-string "Access code you get from page: "))
                         (refresh-token (dropbox-get-refresh-token access-code key sec))
...
(defun dropbox-get-refresh-token (&optional access-code app-key app-secret)
  "If ACCESS-CODE is not null, request from server, else return locally."
  (if access-code
      (dropbox--refresh-token access-code app-key app-secret)
    (alist-get :refresh_token (dropbox-get-config))))

(defun dropbox-get-token ()
...
(defun dropbox-handle:file-exists-p (filename)
  (dropbox-message "[handler] file-exists-p: %s" filename)
...
(defun dropbox-handle:file-directory-p (filename)
  (dropbox-message "[handler] file-directory-p: %s" filename)
...
(defun dropbox-handle:insert-directory (filename _switches &optional _wildcard full-directory-p)
  (dropbox-message "[handler] insert-directory: %s" filename)
  (let* ((filename (expand-file-name filename))
    ...
    (if (not full-directory-p)
      ...
      (pdd-async
        (let* ((reporter (progress-reporter-make "Loading"))
...
(defun dropbox-handle:insert-file-contents (filename &optional visit beg end replace)
  (dropbox-message "[handler] insert-file-contents: %s" filename)
...
(defun dropbox-handle:write-region (beg end filename &optional append visit _lockname _mustbenew)
  (dropbox-message "[handler] write-region: %s, %s, %s" filename beg end)
...
(defun dropbox-handle:file-local-copy (filename)
  (dropbox-message "[handler] file-local-copy: %s" filename)
...
(defun dropbox-handle:dired-insert-directory (dir switches &optional file-list wildcard _hdr)
  (dropbox-message "[handler] dired-insert-directory: %s" dir)
  (if file-list
